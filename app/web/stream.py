...
def stream_camera_mjpg(camera_id: int, request: Request, limpo: bool = False):
    ...
    # `limpo=1`: feed de EXIBIÇÃO (cru, sem bboxes e sem ajuste, na cadência de captura) —
    # a vitrine da feira usa este; o painel de operação usa o anotado (default).
    ...
    if not stream_mod.aguardar_frame_camera(camera_id, limpo=limpo):
        raise HTTPException(
            503,
    ...
    return StreamingResponse(
        stream_mod.gerar_mjpeg_camera(camera_id, limpo=limpo),
        media_type="multipart/x-mixed-replace; boundary=frame",
    )
