...
> seção [Leitura sob demanda](#1-leitura-sob-demanda--get-apileitura)**. O detalhamento
...
- Não há versionamento na URL. O contrato é **aditivo**: campos novos podem aparecer,
  ...
  conhece** — é o que mantém sua integração funcionando entre versões.
...
## 1. Leitura sob demanda — `GET /api/leitura`
...
**No fim do abastecimento** — não antes, não durante. Chamar cedo captura o veículo ainda
...
| `rapido` | não | `1` pede o **modo de captura rápida** — resposta em poucos segundos, lendo menos. Ver [seção 1.1](#11-modo-de-captura-rápida) | Ausente = leitura completa, como sempre |
...
- Configure o **timeout HTTP do cliente para 35–40 segundos**. Um cliente com timeout de
...
`"modo": "completo"`, o modo está desligado no servidor e o motivo aparece em `avisos` —
...
> tem valor — mostrar a placa ao atendente enquanto o carro ainda está na pista, por
...
| `veiculo.combustivel` | string \| null | Só existe se o enriquecimento estiver ligado — ver [seção 2](#2-bloco-veiculo--dados-do-veículo) |
...
`acordo` — se o limiar for recalibrado aqui, sua integração acompanha sozinha.
...
integração. Note que o conjunto de chaves é **diferente** do caso com placa — teste
...
foi desativado" — a correção é diferente em cada caso.
...
## 2. Bloco `veiculo` — dados do veículo
...
### `GET /api/deteccoes` — histórico de leituras

Filtros (todos opcionais): `placa` (busca parcial), `desde`, `ate` (ISO 8601),
`empresa_id`, `bico_id`, `limit` (1–500, padrão 50), `offset`,
`origem` (`producao` — padrão, exclui testes manuais — `teste` ou `todas`),
...
Devolve `{"<placa>": <bloco veiculo> | null}`. **Nunca consulta a API externa** — só o
que já está em cache. Até 500 placas por chamada. `null` = ainda não consultada.

### `GET /api/chamadas` — auditoria das chamadas de leitura

Todas as chamadas recebidas em `/api/leitura`, **inclusive as recusadas**. Filtros:
`limit` (1–500), `empresa_id`, `status`, `apenas_erros=true`.
...
### `GET /api/chamadas/resumo?horas=24` — agregados
...
### `GET /api/listas` — listas branca/negra
...
- **Timeout de 5 segundos**, disparo em thread separada — responda rápido e processe
  depois.
...
Por não ser escopado por posto, o `/ws` é uma ferramenta de diagnóstico/console central —
...
postos — uma URL aberta permitiria varrer a foto mais recente de qualquer bomba de
...
por força bruta, sem login. A api_key **do posto** não abre esta rota — o nome do arquivo
...
Num bico de duas câmeras, `?camera_id=<id>` traz o quadro de uma câmera específica — os
...
2. **`POST /api/bicos/{id}/ler-placa-teste`** — mesma leitura, sem precisar montar a URL
   completa. Devolve o mesmo payload. As detecções ficam marcadas com `origem: "teste"` e
   não poluem a contagem de produção.
3. **A própria `GET /api/leitura`** — pode ser chamada à vontade dentro do rate limit;
...
- [ ] Timeout HTTP do cliente em **35–40 segundos** (~10 s nas chamadas com `rapido=1`).
...
- [ ] Não parsear `mensagem` nem `motivo` — são texto humano.
