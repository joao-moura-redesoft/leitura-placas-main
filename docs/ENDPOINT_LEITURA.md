...
| `cnpj` | sim | CNPJ do posto | Com ou sem pontuação, pois normalizamos para só dígitos |
...
- **Chave errada ou ausente devolve `404`, não `401`.** Isso é proposital: a quem não tem
  a chave, não confirmamos sequer que o cadastro existe.
...
- Configure o **timeout HTTP entre 35 e 40 segundos**. Um cliente com 10s abandona leituras boas
  no meio.
...
| `confianca` | 0 a 1, confiança do OCR na leitura eleita |
| `acordo` | 0 a 1, o número bruto por trás de `confirmada` |
| `tipo_veiculo` | `"moto"`, `"carro"` ou `null`, a **nossa estimativa pela imagem**, não cadastro |
...
não aparece. Quando aparece, traz **sempre as mesmas 16 chaves**, e o que muda é `consulta`:
...
desativado", porque a correção é diferente em cada caso.
...
| Timeout HTTP sugerido | 35 a 40 s | ~10 s |
...
> **Qual usar.** O modo rápido serve a fluxos em que resposta tardia não tem valor, como mostrar
...
- [ ] Timeout HTTP entre **35 e 40 s** (ou ~10 s se usar `rapido=1`)
...
- [ ] **Ignorar chaves desconhecidas**, porque o contrato é aditivo: campos novos podem aparecer,
